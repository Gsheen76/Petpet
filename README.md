...
- **小屋窗口恢复置顶**，不被其他窗口挡住。
- **存档一键备份与恢复**：设置页新增「存档备份」——一键备份、导出副本、从备份恢复（恢复前自动做安全快照，完成后程序自动重启；不含 API Key，自动保留最近 5 份）。
- **开机自动启动**：设置页新增开关，开机后自动把 TA 叫到桌面上（仅 Windows）。
- **聊天可以停止和重新生成**：回复太长时点「停止」立即截断保留已收到的部分；点「↻」重新生成上一条回复。
