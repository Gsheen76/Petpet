...
- 新增“小游戏中心”，包含金币雨和幸运爪爪两种可重复游玩的 Pet 币玩法
- 新增小游戏成绩、最佳分数、局数和 Pet 币收入记录，并接入成就系统
- 参数调试器支持源码模式实时调节窗口、物理、动画、成长和小游戏参数
- 聊天界面统一字体大小，未配置 API Key 时显示状态文字和提醒红点
- 优化小狗底纹、字体可读性、桌宠可见性和多屏位置恢复
- 喂食动画按完整两轮播放后停止，进食时长与调试参数保持一致
- Windows 更新增加 TLS 握手重试、友好超时提示和备份文件清理保障
- 修复更新替换成功后残留备份文件、下载失败显示原始 SSL 异常等问题
