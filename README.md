...
- **宠物面板「查看 / 陪伴」分离**：点头像只切换查看（简介、套装、送礼、改名都跟着查看对象走），点「陪我」才真正换小狗出勤；名牌、陪我键、改名组件全面换成定稿素材，好感升级有提示。
- **送礼有反应**：送出成功的那一刻，出勤小狗当场开心表演（播放玩耍动画）。
- **聊天搜索 + 陪伴周报**：聊天窗新增搜索框，按关键词即时过滤历史；记录页顶部新增陪伴周报卡。
- **礼物排版统一**：面板礼物页三列恒等宽（不再偏右），滚动条常驻。
- **存档更安全**：原子写入 + 自动备份——断电、崩溃或磁盘故障不再导致进度清零，主档损坏时自动回退上一份完好存档。
- **聊天更顺更稳**：长期记忆抽取移入后台线程（此前长会话每条消息可能卡顿最长 20 秒）；档案不再随使用自我堆积重复条目；对话异常时如实报错，输入框不再永久卡死。
- **自动睡醒回归**：精力过低会自己走去睡觉、睡饱自动醒来（该功能自 v1.5.0 起意外失效，本版修复）。
- **家园与交互打磨**：家具旁走过的层级闪烁按实测日志二次校准根除；双键确认弹窗按 Esc = 取消（送礼不再误触送出）；新家具不再能被拖出全景边界；小游戏按键补齐全应用统一反馈、金币雨截止后的点击不再计分。
- 全量 882 项自动化测试，GitHub Actions 每次 push 自动回归。

## v1.7.3 更新亮点

- **家具大扩充（8→12 件）**：新增复古圆挂钟、猫爬架、宠物软垫小床、木质小摇椅；全部家具按新画风整套重制，抠图升级（接触阴影吸收，脚下不再残留白边），并统一由引擎绘制接地软阴影——白家具也稳稳「落地」。
- **小狗状态卡全新设计**：换成与家具同风格的暖木挂牌，饱腹 / 心情 / 精力三条进度条实时画进凹槽，文字按显示分辨率渲染清晰锐利。
- **生日 / 纪念日主动提醒**：档案「重要的事」里记着的日子（如「生日是6月18日」）到了当天，TA 会主动来祝贺（每条重要的事当天只提一次）；搭话新增四季话术与公历节日问候。
- **聊天记录一键导出**：聊天窗新增「导出」，把全部对话存成 txt 文本（防手滑清空）；档案六栏支持导出 / 导入 JSON。
- **购买家具默认进收纳**：新家具先收进仓库，从装修模式自行摆放；装修面板支持滚轮滚动（家具多了不再截断），家具遮挡按脚底线排序 + 临界滞回防闪烁，拖拽中的家具置顶显示。
- **内存占用减半**：贴图按显示尺寸预缩 + 帧预算化，实测 347MB → 174MB；落地弹跳更顺滑（连续碰撞消除触底停顿），触底音效按用户反馈移除。
- **修复闪退**：升级时刻偶发的崩溃（三天三次）根因修复——飘字气泡被内存回收后定时器悬空调用。
- **聊天回复不再中途换行**，午餐肉聊天头像恢复整身原版（耳尖完整入框）。
- 全量 856 项自动化测试，GitHub Actions 每次 push 自动回归。

## v1.7.1 更新亮点

- **礼物系统**：商店新增「礼物」页（三档 × 每档三选一共 9 种，全宠物中性），买了送礼涨好感；宠物面板新增第三分栏管理背包与赠送，每只小狗每档各有最爱（好感 ×1.5）。送礼成就同步上线（购买/送出共 7 项梯度）。
- **游戏中自动隐藏**：检测到其他游戏前台运行时小狗自动藏起、游戏退出自动回来；设置页新增开关（默认开启）。
- **右键菜单全新图标**：三页 18 个按键全部换上哑光奶油贴图，图标即按键——悬浮显示功能名，界面更干净。
- **待机更从容**：三项属性消耗速度整体下调，无强化可待机约 2 小时，「持久活力」强化满级约 4 小时。
- **界面细节**：商店页签栏换 5 槽新素材、礼物分栏复用 4 槽素材（槽线对齐）、滚动条贴边统一、聊天宠物头像改为大头照。
...
- 家园交互全面提速：宠物注册表、墙面状态卡与家具变换全部缓存化，画一帧从约 100ms 降到约 2ms，点地移动、按钮点击、开小屋都立即可响应。
- 胶囊按键手感升级：悬停放大+白洗+描边，按下缩小+灰黑，点击动作在反馈播放后触发，不再残留。
...
```powershell
.\scripts\release.ps1 -Version 1.7.1
```
