...
- 新增温馨记录，持续统计陪伴时长、互动次数、Pet币收支和升级数据
- 新增暖心成就与等级奖励，完成目标后可领取 Pet币
- 新增 Pet币商店，包含装饰与强化两个栏目
- 新增好感度等级；互动提高好感，好感决定实时 `EXP/min`
- 抚摸、喂食、玩耍、睡眠和成长速度均可逐级强化
- 新增免费“暖心红项圈”，支持领取、装备和卸下
- 右键成长卡加入 Pet币、好感进度和每分钟经验信息
- 完成运行素材与项目目录整理，修复发布包可能遗漏接球小球素材的问题
...
- 装饰栏目已支持领取、购买、装备和卸下，并提供免费的“暖心红项圈”。
  由于现有逐帧动画没有统一骨骼，装扮目前只在可靠的静止姿态显示，
  走路、睡眠、进食、抚摸和接球等复杂动作期间会自动隐藏，结束后恢复。
...
2. 双击小狗打开聊天窗口。
3. 点击聊天框底部的“添加 API Key”，在密码框中填入并保存。下一次聊天会立即生效，不需要重启。

聊天窗口底部还可以选择聊天模型并清除对话记忆。当前提供
`GLM-4-Flash`，所选模型会与 API Key 一起保存在本机配置中。
...
- 打开聊天窗口，检查底部按钮是否显示“API Key：已配置”。
- 如果显示“API Key：环境变量”，程序会优先使用
  `ZHIPU_API_KEY`，可检查该环境变量是否仍然有效。
- 检查网络是否能访问智谱开放平台。
