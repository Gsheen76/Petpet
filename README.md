...
- 新增 24 帧抚摸动画，点击后会出现小手轻抚小狗头部
- 重做互动扔球玩法，小球和正面扑跃的小狗同步运动并在所选落点交会
- 接球后定格 1 秒，并显示成功气泡、爱心、星星和光环庆祝效果
- 精力低于 30% 时会自主走到最近的屏幕角落睡觉，自动睡眠恢复到 80% 以上后自行醒来
- 手动睡眠继续完全由玩家控制，不会被自动作息逻辑唤醒
- 聊天窗口可直接配置 API Key、选择模型和清除记忆
- 饥饿与低心情气泡改为更温馨、清晰且不依赖表情字体的胶囊样式
