...
| 领域 | 内容 |
|------|------|
| **纠偏：套装键还原 + 商店二分栏 + 档案弹窗撑满**（2026-09-11 第二批） | 用户指正素材表用途：① **tab_outfit.png 原样还原**（烘焙稿撤下，勿再动宠物面板分栏键）；② 素材表 bar1（2 格）→ **刻度归一化到精确 50%** 后落 `shop/chip_bar_2seg.png`，接 `QFrame#outfitPetSelector` border-image 专属规则（照 giftFilterBar 先例），套装页宠物选择器撤 chipBar 属性（boundary 测试同步）；③ **档案弹窗 620×760 → 850×960 撑满整页**：字号全面放大（标题 30/栏名 24/输入 20/底部键 20），六卡 Expanding 均分填满高度。验证：受影响 59 测试绿 + 实平台渲染验收（二分栏药丸托盘圆头/刻度正常、弹窗六卡填满无大空白）+ 全量后台复跑 + 重启 PID 36964。 |
