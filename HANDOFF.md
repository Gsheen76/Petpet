...
| 领域 | 内容 |
|------|------|
| **商店徽标+反馈统一** | 免费/使用中/已拥有素材缩 5%（47→45、50→48）；四页页首小字删除；`FeedbackButton`（商店全部交互键基类）升级现行反馈：悬停白洗+珊瑚描边、按住缩小压暗、非 checkable 键拦截原生 click 回弹 40ms 后触发（checkable 保留原生时序防破坏选中态）（`e8d4da8`） |
