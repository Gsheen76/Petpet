...
def test_macos_build_uses_lightweight_version_module():
    ...
    assert re.search(r'^VERSION = "1\.7\.3"$', (
        ROOT / "version.py"
...
def test_readme_documents_one_click_release_and_v170_assets():
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    assert ".\\scripts\\release.ps1 -Version 1.7.1" in readme
    for asset in (
        "Petpet.exe",
        "Petpet-v1.7.3-windows.zip",
        "Petpet-v1.7.3-macOS-arm64.zip",
    ):
        assert asset in readme
    assert "-macOS-intel.zip" not in readme
